...
- `sentSelfMessage` suppression: TS3 fires `onTextMessageEvent` **twice** for a message sent via `requestSendChannelTextMsg` from inside a callback — once synchronously (immediate) and once when the server echoes it back. The `sentSelfMessage=1` else branch returns `1` (suppress) to eat the synchronous callback; the server echo then fires with `sentSelfMessage=0`, fails the URL check, and returns `0` to display the message exactly once. Changing that `return 1` back to `return 0` causes every formatted message to post twice.
