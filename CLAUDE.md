...
**Plugin metadata (version, description) lives in the vcxproj, not in the header.**
`plugin_version.h` is auto-generated by the `GeneratePluginVersionHeader` MSBuild target on every build — editing it directly is pointless. To change the version or description, update the `PluginInfo` PropertyGroup in `ts3websitepreview/ts3websitepreview.vcxproj`. The same values are substituted into `package.ini` at build time.
